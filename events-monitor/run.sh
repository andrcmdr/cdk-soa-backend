...
docker run --name nats_2 -v ./.natsdata/:/apps/nats.db/ -v ./.logs/:/apps/.logs/ -v ./.config/nats.config:/apps/.config/nats.config:ro --network host -p 127.0.0.1:4222:4222 -p 127.0.0.1:4242:4242 -d cdk-soa-backend ./nats-server --name 'events_monitor_bus_nats_server' --addr 127.0.0.1 --port 4222 --http_port 4242 --config ./.config/nats.config --log_size_limit 1073741824 --jetstream
