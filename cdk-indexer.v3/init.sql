...
CREATE INDEX IF NOT EXISTS idx_chain_id ON contract_events (chain_id);
...
CREATE INDEX IF NOT EXISTS idx_chain_id_block_number_hash_timestamp ON contract_events (chain_id, block_number, block_hash, block_timestamp);

CREATE INDEX IF NOT EXISTS idx_transaction_hash ON contract_events (transaction_hash);
CREATE INDEX IF NOT EXISTS idx_transaction_sender ON contract_events (transaction_sender);
CREATE INDEX IF NOT EXISTS idx_transaction_receiver ON contract_events (transaction_receiver);
...
CREATE INDEX IF NOT EXISTS idx_log_hash ON contract_events (log_hash);
CREATE INDEX IF NOT EXISTS idx_transaction_hash_sender_index_log_index_hash ON contract_events (transaction_hash, transaction_sender, transaction_receiver, transaction_index, log_index, log_hash);
...
CREATE INDEX IF NOT EXISTS idx_contract_chain_block_tx_log_event ON contract_events (contract_name, contract_address, chain_id, block_number, block_hash, block_timestamp, transaction_hash, transaction_sender, transaction_receiver, transaction_index, log_index, log_hash, event_name, event_signature);
